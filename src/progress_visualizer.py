def to_console(logger, name, epoch, it, losses, performances, n_iters_per_epoch=1, lr=None):
    progress_str = "\n" + "-" * 60 + "\n"
...
def to_tensorboard(tb_writer, name, epoch, it, losses, performances, n_iters_per_epoch=1, lr=None):
...
class ProgressVisualizer:
    ...
    def display(self, epoch, it, losses, performances, lr=None):
        if (it + 1) % self.show_progress_every_n_iters == 0:
            for display_func, handler in zip(self.display_funcs, self.handlers):
                display_func(
                    handler, self.name, epoch, it, losses, performances, n_iters_per_epoch=self.n_iters_per_epoch, lr=lr
                )
