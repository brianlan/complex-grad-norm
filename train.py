...
parser.add_argument("--lr-init", default=0.005, type=float)
...
def main(args):
    grad_normalizer = SimpleGradNormalizer(lr_init=args.lr_init, alpha=1.0) if args.gradnorm else None
...
def train_phase(epoch, model, dataloader, optimizer, visualizer, grad_normalizer=None):
    ...
    for i, (images, gt_classes, gt_bboxes, gt_counts) in enumerate(dataloader):
        ...
        if grad_normalizer is not None:
            ...
            visualizer.display(epoch, i, losses, performances, other_metrics=grad_normalizer.loss_weight, lr=get_current_lr(optimizer))
        else:
